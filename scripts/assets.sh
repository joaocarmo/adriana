#!/usr/bin/env bash
#
# Encrypt private assets (her photograph) so they can live in a public repo,
# and decrypt them at build time. The passphrase lives in the ASSETS_KEY secret.
#
...
# Both directions must use identical flags, so they are declared once.
